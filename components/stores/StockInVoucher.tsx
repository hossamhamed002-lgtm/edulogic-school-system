...
const StockInVoucher: React.FC = () => {
  ...
  const { activeSchool } = (window as any).store || { activeSchool: null };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [vouchersRes, seqRes] = await Promise.all([
        ...
        const [vouchersData, seqData] = await Promise.all([
        ...
        console.warn('[API][WARN] stock-in fetch failed; using local copy.', err);
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn('[API][WARN] stock-in persist failed.', err);
      }
    })();
  }, [vouchers, seq, schoolCode]);
