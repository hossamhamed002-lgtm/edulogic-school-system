...
const loadAuditLogs = async (schoolId?: string): Promise<AuditLogEntry[]> => {
  try {
    if (schoolId) {
      const res = await fetch(`${API_BASE}/audit/logs/${encodeURIComponent(schoolId)}`);
      if (res.ok) {
...
const persistAuditLogs = async (entries: AuditLogEntry[], schoolId?: string) => {
  const trimmed = entries.slice(-MAX_ENTRIES);
  try {
    if (schoolId) {
      await fetch(`${API_BASE}/audit/logs/${encodeURIComponent(schoolId)}`, {
        method: 'POST',
...
export const logAuditEvent = async (partial: Partial<AuditLogEntry>) => {
  ...
  const logs = await loadAuditLogs(entry.schoolId);
  logs.push(entry);
  await persistAuditLogs(logs, entry.schoolId);
  return entry;
};

export const getAuditLogs = async (schoolId?: string) => loadAuditLogs(schoolId);
