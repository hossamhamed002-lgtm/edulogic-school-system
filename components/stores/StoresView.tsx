...
const INVENTORY_TYPES_ENDPOINT = '/stores/types';
const INVENTORY_TYPE_SEQ_ENDPOINT = '/stores/types-seq';
const INVENTORY_AUTO_SEQ_ENDPOINT = '/stores/auto-seq';
const FIXED_ASSETS_ENDPOINT = '/stores/assets';
const FIXED_ASSET_SEQ_ENDPOINT = '/stores/assets-seq';
...
const StoresView: React.FC<{ store: any }> = ({ store }) => {
  ...
  const [assetSeq, setAssetSeq] = useState<number>(0);
  ...
  const [typeSeq, setTypeSeq] = useState<number>(0);
  const [autoAccSeq, setAutoAccSeq] = useState<number>(0);
  ...
  // جلب بيانات المخازن من الباك-إند
  useEffect(() => {
    if (!schoolCode) return;
    let cancelled = false;
    (async () => {
      try {
        const [typesRes, typesSeqRes, autoSeqRes, assetsRes, assetSeqRes] = await Promise.all([
          fetch(`${API_BASE}${INVENTORY_TYPES_ENDPOINT}/${encodeURIComponent(schoolCode)}`),
          fetch(`${API_BASE}${INVENTORY_TYPE_SEQ_ENDPOINT}/${encodeURIComponent(schoolCode)}`),
          fetch(`${API_BASE}${INVENTORY_AUTO_SEQ_ENDPOINT}/${encodeURIComponent(schoolCode)}`),
          fetch(`${API_BASE}${FIXED_ASSETS_ENDPOINT}/${encodeURIComponent(schoolCode)}`),
          fetch(`${API_BASE}${FIXED_ASSET_SEQ_ENDPOINT}/${encodeURIComponent(schoolCode)}`)
        ]);
        const [typesData, typesSeqData, autoSeqData, assetsData, assetSeqData] = await Promise.all([
          typesRes.ok ? typesRes.json() : [],
          typesSeqRes.ok ? typesSeqRes.json() : 0,
          autoSeqRes.ok ? autoSeqRes.json() : 0,
          assetsRes.ok ? assetsRes.json() : [],
          assetSeqRes.ok ? assetSeqRes.json() : 0
        ]);
        if (!cancelled) {
          setInventoryTypes(Array.isArray(typesData) ? typesData : []);
          setTypeSeq(Number(typesSeqData) || 0);
          setAutoAccSeq(Number(autoSeqData) || 0);
          setFixedAssets(Array.isArray(assetsData) ? assetsData : []);
          setAssetSeq(Number(assetSeqData) || 0);
        }
      } catch (err) {
        console.warn('[API][WARN] stores fetch failed; using local copy.', err);
      }
    })();
    return () => {
      cancelled = true;
  ...
  // حفظ بيانات المخازن في الباك-إند
  useEffect(() => {
    if (!schoolCode) return;
    (async () => {
      try {
        await Promise.all([
          ...
          fetch(`${API_BASE}${INVENTORY_TYPE_SEQ_ENDPOINT}/${encodeURIComponent(schoolCode)}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ value: typeSeq || 0 })
          }),
          fetch(`${API_BASE}${INVENTORY_AUTO_SEQ_ENDPOINT}/${encodeURIComponent(schoolCode)}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ value: autoAccSeq || 0 })
          }),
          fetch(`${API_BASE}${FIXED_ASSETS_ENDPOINT}/${encodeURIComponent(schoolCode)}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(fixedAssets || [])
          }),
          fetch(`${API_BASE}${FIXED_ASSET_SEQ_ENDPOINT}/${encodeURIComponent(schoolCode)}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ value: assetSeq || 0 })
          })
        ]);
      } catch (err) {
        console.warn('[API][WARN] stores persist failed.', err);
      }
    })();
  }, [inventoryTypes, typeSeq, autoAccSeq, fixedAssets, assetSeq, schoolCode]);
