...
export const useAccounts = () => {
  ...
  // fetch accounts from API (or seed if empty)
  useEffect(() => {
    if (!schoolCode) return;
    let cancelled = false;
    (async () => {
      try {
        const res = await fetch(`${API_BASE}/finance/accounts/${encodeURIComponent(schoolCode)}`);
        if (res.ok) {
  ...
  useEffect(() => {
    setAccounts((prev) => {
      let expensesRoot = prev.find(
        (account) => account.id === 'ACC-ROOT-EXPENSES' || (account.code === '5' && account.parentId === null)
      );
      const existingCodes = new Set(prev.map((account) => account.code));
      const additions: Account[] = [];
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await fetch(`${API_BASE}/finance/accounts/${encodeURIComponent(schoolCode)}`, {
          method: 'POST',
