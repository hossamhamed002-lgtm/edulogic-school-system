...
import { AccountType, AccountLevel } from '../../src/types/accounts.types';
...
type StockIssueStatus = 'DRAFT' | 'POSTED' | 'VOID';
...
const mockItems = ['أقلام', 'دفاتر', 'كتب', 'أوراق', 'أحبار', 'أجهزة حاسب', 'قطع غيار', 'مقاعد', 'طابعات', 'أدوات معمل'];

const StockIssueVoucher: React.FC<{ store: any }> = ({ store }) => {
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | StockIssueStatus>('all');
  const [filterType, setFilterType] = useState<string>('');
  const [showModal, setShowModal] = useState(false);
  const [mode, setMode] = useState<'add' | 'edit'>('add');
  const [voucher, setVoucher] = useState<Omit<StockIssueVoucher, 'lines' | 'totals'>>({
    id: '',
    code: '',
  ...
  const [lines, setLines] = useState<StockIssueLine[]>([
    { id: 'l1', itemId: '', itemName: '', qty: 0, unit: '', unitCost: 0, totalCost: 0 }
  ]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [typesRes, vouchersRes, seqRes] = await Promise.all([
          fetch(`${API_BASE}${ENDPOINT_TYPES}/${encodeURIComponent(schoolCode)}`),
          fetch(`${API_BASE}${ENDPOINT_ISSUE}/${encodeURIComponent(schoolCode)}`),
          fetch(`${API_BASE}${ENDPOINT_VOUCHER_SEQ}/${encodeURIComponent(schoolCode)}`)
        ]);
        const [typesData, vouchersData, seqData] = await Promise.all([
          typesRes.ok ? typesRes.json() : [],
          vouchersRes.ok ? vouchersRes.json() : [],
          seqRes.ok ? seqRes.json() : 0
        ]);
        if (!cancelled) {
          setInventoryTypes(Array.isArray(typesData) ? typesData : []);
          setVouchers(Array.isArray(vouchersData) ? vouchersData : []);
          setSeq(Number(seqData) || 0);
        }
      } catch (err) {
        console.warn('[API][WARN] stock issue fetch failed; using local copy.', err);
      }
    })();
    return () => {
      cancelled = true;
      ...
      try {
        await Promise.all([
          fetch(`${API_BASE}${ENDPOINT_TYPES}/${encodeURIComponent(schoolCode)}`, {
            ...
            body: JSON.stringify(inventoryTypes || [])
          }),
          fetch(`${API_BASE}${ENDPOINT_ISSUE}/${encodeURIComponent(schoolCode)}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          fetch(`${API_BASE}${ENDPOINT_VOUCHER_SEQ}/${encodeURIComponent(schoolCode)}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ value: seq || 0 })
          })
        ]);
      } catch (err) {
        console.warn('[API][WARN] stock issue persist failed.', err);
      }
    })();
  }, [inventoryTypes, vouchers, seq, schoolCode]);
  ...
  const generateCode = () => {
    ...
    let exists = true;
    while (exists) {
      code = `ISU-${year}-${String(next).padStart(6, '0')}`;
  ...
  const saveDraft = () => {
    ...
    const payload: StockIssueVoucher = {
      id: mode === 'edit' && voucher.id ? voucher.id : `ISU-${Date.now()}`,
      ...voucher,
  ...
  const buildPostingAccounts = () => {
    ...
    let debitId = type?.expenseAccountId || '';
    let creditId = type?.inventoryAccountId || '';
    let updated = type ? { ...type } : undefined;

    if (!creditId) {
      const invAcc = ensureAccount(`مخزون - ${type?.name || 'تشغيلي'}`, AccountType.ASSET);
      creditId = invAcc?.id || '';
      if (updated) updated.inventoryAccountId = creditId;
    }
    if (!debitId) {
      const expAcc = ensureAccount(`صرف مخزون - ${type?.name || 'تشغيلي'}`, AccountType.EXPENSE);
      debitId = expAcc?.id || '';
      if (updated) updated.expenseAccountId = debitId;
    }

    if (type && updated && (updated.inventoryAccountId !== type.inventoryAccountId || updated.expenseAccountId !== type.expenseAccountId)) {
  ...
  const postVoucher = () => {
    ...
    const journal: JournalEntry = {
      ...
      journalNo: Date.now(),
      date: payload.date,
      description: payload.description || `ترحيل إذن صرف ${payload.code}`,
      source: 'manual',
      ...
      createdBy: userName,
      lines: [
        { id: 'DR', accountId: debitId, debit: totals.cost, credit: 0, note: 'صرف مخزون' },
        { id: 'CR', accountId: creditId, debit: 0, credit: totals.cost, note: 'تخفيض المخزون' }
      ],
  ...
  const exportCsv = () => {
    ...
    link.href = url;
    link.download = `stock-issues-${schoolCode}-${activeYear}.csv`;
    link.click();
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6" id="issue-print-area">
        <div className="flex flex-col gap-3">
          <div>
            <h2 className="text-2xl font-black text-slate-900">إذن صرف مخزن</h2>
            <p className="text-sm text-slate-500 font-bold">صرف أصناف من المخزن مع ترحيل محاسبي تلقائي حسب نوع المخزون</p>
          </div>
          <div className="flex flex-wrap items-center gap-2 justify-end">
            ...
            <button
              ...
              onClick={() => {
                ...
                setVouchers((prev) =>
                  prev.map((v) => (v.id === current.id ? { ...v, status: 'VOID' as StockIssueStatus } : v))
                );
                setSelectedId(null);
              }}
            ...
            <button
              onClick={() => exportUtils.print('issue-print-area', 'landscape', 8)}
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 w-full max-w-6xl max-h-[92vh] overflow-hidden flex flex-col" dir="rtl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <div>
                <p className="text-[11px] uppercase tracking-[0.3em] text-slate-400">Stock Issue</p>
                <h3 className="text-xl font-black text-slate-900">{mode === 'add' ? 'إضافة إذن صرف مخزون' : 'تعديل إذن صرف مخزون'}</h3>
              </div>
              <button
                        ...
                        <td className="p-2">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              setLines((prev) =>
                                prev.map((l) => (l.id === line.id ? { ...l, qty, totalCost: (l.unitCost || 0) * qty } : l))
                              );
                            }}
                        ...
                        <td className="p-2">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              setLines((prev) =>
                                prev.map((l) =>
                                  l.id === line.id ? { ...l, unitCost, totalCost: (l.qty || 0) * unitCost } : l
                                )
                              );
                            }}
                            className="w-full rounded-lg border border-slate-200 px-2 py-1 text-sm text-emerald-700 font-bold"
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {(() => {
                      const { debitId, creditId } = buildPostingAccounts();
                      const preview = [
                        debitId && { id: 'dr', accountId: debitId, debit: totals.cost, credit: 0, note: 'صرف مخزون' },
                        creditId && { id: 'cr', accountId: creditId, debit: 0, credit: totals.cost, note: 'تخفيض مخزون' }
