  ...
  const applyDefaultStructure = async () => {
    ...
    try {
      (existingClasses || []).forEach((cls: any) => {
        if (cls?.Class_ID) deleteClass?.(cls.Class_ID);
      });
      (existingGrades || []).forEach((gr: any) => {
        if (gr?.Grade_ID) deleteGrade?.(gr.Grade_ID);
      });
      (existingStages || []).forEach((stg: any) => {
    ...
    const askSections = (gradeName: string, fallback: number) => {
      const promptLabel = lang === 'ar'
        ? `أدخل عدد الفصول لصف "${gradeName}" (يجب أن يكون 1 على الأقل):`
        : `Enter number of sections for "${gradeName}" (minimum 1):`;
      ...
      return Math.min(parsed, 30); // سقف منطقي
    };
    ...
    for (const stg of template) {
      ...
      for (const grd of stg.grades) {
        ...
        if (sectionCount <= 0) continue; // لا تُنشئ فصول إذا كان العدد صفر
