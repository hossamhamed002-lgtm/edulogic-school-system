...
export const useTreasuryLogic = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${API_BASE}/finance/banks/${encodeURIComponent(schoolCode)}`);
        if (res.ok) {
          const data = await res.json();
          if (!cancelled && Array.isArray(data)) setTreasuryAccounts(data as TreasuryAccount[]);
        }
      } catch {
        /* ignore */
      }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await fetch(`${API_BASE}/finance/banks/${encodeURIComponent(schoolCode)}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(treasuryAccounts || [])
        });
      } catch {
        /* ignore */
      }
    })();
  }, [treasuryAccounts, schoolCode]);
