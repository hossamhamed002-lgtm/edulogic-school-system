...
import { fetchFinancialCloseState, persistFinancialCloseState } from '../../src/utils/financialYearClose';
...
const FinancialYearClose: React.FC<{ store: any }> = ({ store }) => {
  ...
  const [closeState, setCloseState] = useState<CloseState>({ isClosed: false });
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (remote) setCloseState(remote as CloseState);
    })();
    return () => {
      cancelled = true;
    };
  ...
  const handleClose = () => {
    ...
    persistFinancialCloseState(nextState, schoolCode, yearId);
