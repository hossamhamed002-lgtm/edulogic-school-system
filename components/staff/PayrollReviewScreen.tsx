...
const ENDPOINT_OPERATIONAL = '/hr/operational';
const ENDPOINT_PAYROLL_DRAFT = '/hr/payroll-draft';
...
const PayrollReviewScreen: React.FC<PayrollReviewScreenProps> = ({ store, onBack }) => {
  ...
  const [operationalRecords, setOperationalRecords] = useState<Record<string, { salary?: string; contractType?: string; jobTitleId?: string }>>({});
  const [draftByEmployee, setDraftByEmployee] = useState<Record<string, PayrollDraftRow>>({});
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [opRes, draftRes] = await Promise.all([
          fetch(`${API_BASE}${ENDPOINT_OPERATIONAL}/${encodeURIComponent(schoolCode)}`),
          fetch(`${API_BASE}${ENDPOINT_PAYROLL_DRAFT}/${encodeURIComponent(schoolCode)}`)
        ]);
        const [opData, draftData] = await Promise.all([
          opRes.ok ? opRes.json() : {},
          draftRes.ok ? draftRes.json() : {}
        ]);
        if (!cancelled) {
          setOperationalRecords(opData && typeof opData === 'object' ? opData : {});
          setDraftByEmployee(draftData && typeof draftData === 'object' ? draftData : {});
        }
      } catch (err) {
        console.warn('[API][WARN] payroll/operational fetch failed; using local copy.', err);
      }
    })();
    return () => {
      cancelled = true;
      ...
      try {
        await Promise.all([
          fetch(`${API_BASE}${ENDPOINT_OPERATIONAL}/${encodeURIComponent(schoolCode)}`, {
            ...
            body: JSON.stringify(operationalRecords || {})
          }),
          fetch(`${API_BASE}${ENDPOINT_PAYROLL_DRAFT}/${encodeURIComponent(schoolCode)}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(draftByEmployee || {})
          })
        ]);
      } catch (err) {
        console.warn('[API][WARN] payroll/operational persist failed.', err);
      }
    })();
  }, [operationalRecords, draftByEmployee, schoolCode]);
