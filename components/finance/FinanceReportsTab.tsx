...
import {
  ...
  financialCloseStorageKey,
  fetchFinancialCloseState
} from '../../src/utils/financialYearClose';
...
const FinanceReportsTab: React.FC<FinanceReportsTabProps> = ({ store }) => {
  ...
  const [isYearClosed, setIsYearClosed] = useState(() => isFinancialYearClosed(schoolCode, yearId));
  ...
  const loadReportSettings = (reportId: string) => {
    ...
    return (all && all[reportId]) || defaultReportSettings;
  };
  ...
  const handleSaveSettings = () => {
    ...
    const rawAll = getReportSettings(schoolCode);
    const next = { ...(rawAll || {}), [settingsReportId]: reportSettings };
  ...
  React.useEffect(() => {
    let cancelled = false;
    (async () => {
      const remote = await fetchFinancialCloseState(schoolCode, yearId);
      if (cancelled) return;
      if (remote) {
        setCloseState(remote);
        if (remote.isClosed) setIsYearClosed(true);
      }
    })();
    return () => {
      cancelled = true;
