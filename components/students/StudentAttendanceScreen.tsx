...
const StudentAttendanceScreen: React.FC<StudentAttendanceScreenProps> = ({ store }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${API_BASE}/students/attendance/${encodeURIComponent(schoolCode)}`);
        if (res.ok) {
          const data = await res.json();
          if (!cancelled && data && typeof data === 'object') setRecords(data);
        }
      } catch {
        /* ignore */
      }
  ...
  useEffect(() => {
    (async () => {
      try {
        await fetch(`${API_BASE}/students/attendance/${encodeURIComponent(schoolCode)}`, {
          method: 'POST',
