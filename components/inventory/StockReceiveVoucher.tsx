...
const StockReceiveVoucher: React.FC<{ store: any }> = ({ store }) => {
    ...
    (async () => {
      try {
        const [typesRes, vouchersRes, seqRes] = await Promise.all([
          ...
          fetch(`${API_BASE}${ENDPOINT_RECEIVE}/${encodeURIComponent(schoolCode)}`),
        ...
        console.warn('[API][WARN] stock receive fetch failed; using local copy.', err);
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn('[API][WARN] stock receive persist failed.', err);
      }
    })();
  ...
  useEffect(() => {
    ...
    // sequence is persisted via API; we already setSeq(next) and use effect persists
