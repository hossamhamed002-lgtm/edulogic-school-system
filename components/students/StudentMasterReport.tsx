...
const StudentMasterReport: React.FC<{ store: any }> = ({ store }) => {
  ...
  useEffect(() => {
    const all = getReportSettings(activeSchool?.School_Code || activeSchool?.code || 'DEFAULT');
    const current = (all && all[SETTINGS_KEY]) || defaultSettings;
    setSettings({ ...defaultSettings, ...current });
  }, [activeSchool]);

  const saveSettings = () => {
    const all = getReportSettings(activeSchool?.School_Code || activeSchool?.code || 'DEFAULT');
    const next = { ...(all || {}), [SETTINGS_KEY]: settings };
    saveReportSettings(activeSchool?.School_Code || activeSchool?.code || 'DEFAULT', next);
