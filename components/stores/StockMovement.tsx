...
const StockMovement: React.FC<{ store: any }> = ({ store }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [typesRes, recRes, issRes] = await Promise.all([
        ...
        const [typesData, recData, issData] = await Promise.all([
          typesRes.ok ? typesRes.json() : [],
          recRes.ok ? recRes.json() : [],
          issRes.ok ? issRes.json() : []
        ]);
        if (!cancelled) {
          setInventoryTypes(Array.isArray(typesData) ? typesData : []);
          setReceives(Array.isArray(recData) ? recData : []);
          setIssues(Array.isArray(issData) ? issData : []);
        }
      } catch (err) {
        console.warn('[API][WARN] stock movement fetch failed; using local copy.', err);
      }
    })();
    return () => {
      cancelled = true;
