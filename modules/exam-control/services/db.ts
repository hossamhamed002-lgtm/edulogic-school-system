...
// تخزين ذاكرة بديل يمنع أي كتابة على localStorage الحقيقي
...
const API_BASE = (import.meta as any)?.env?.VITE_EXAM_BACKEND_URL || 'http://localhost:4000';
...
const loadLocalSchoolInfo = (): SchoolInfo => {
  ...
  // fire-and-forget refresh from backend
...
const loadLocalStudents = (year: string): Student[] => {
  ...
  refreshStudentsFromBackend(year);
