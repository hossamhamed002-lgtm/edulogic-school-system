...
const Header: React.FC<HeaderProps> = ({ store }) => {
  const { t, toggleLang, activeYear, years, setActiveYearId, exportData, importData, lang, isSaved, activeSchool, programmerMode, exitProgrammerMode } = store;
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="text-start">
          ...
          <div className="flex items-center gap-4 mt-0.5">
            ...
            <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-lg border ${isSaved ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : 'bg-amber-50 border-amber-100 text-amber-600 animate-pulse'}`}>
               {isSaved ? <CheckCircle2 size={12} /> : <CloudUpload size={12} />}
               <span className="text-[9px] font-black uppercase tracking-tighter">
                 {isSaved ? (isRtl ? 'تم الحفظ' : 'Saved') : (isRtl ? 'جاري التأمين...' : 'Securing...')}
               </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 bg-slate-50 p-1 rounded-2xl border border-slate-100 me-2">
           <button 
             onClick={exportData}
             className="p-2 hover:bg-white rounded-xl text-emerald-600 transition-all flex items-center gap-2 px-3 group"
             title={isRtl ? "تنزيل الخزنة" : "Download Vault"}
           >
           ...
           <button 
             ...
             className="p-2 hover:bg-white rounded-xl text-amber-600 transition-all flex items-center gap-2 px-3 group"
             title={isRtl ? "رفع نسخة سابقة" : "Restore Backup"}
           >
