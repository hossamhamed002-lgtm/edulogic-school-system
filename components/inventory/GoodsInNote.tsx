...
const ENDPOINT_GOODS_IN = '/stores/goods-in';
    ...
    (async () => {
      try {
        const [typesRes, vouchersRes, seqRes] = await Promise.all([
          ...
          fetch(`${API_BASE}${ENDPOINT_GOODS_IN}/${encodeURIComponent(schoolCode)}`),
        ...
        console.warn('[API][WARN] goods-in fetch failed; using local copy.', err);
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn('[API][WARN] goods-in persist failed.', err);
      }
    })();
